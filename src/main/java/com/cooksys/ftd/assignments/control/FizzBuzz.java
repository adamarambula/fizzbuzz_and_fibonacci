...
public class FizzBuzz {
    ...
    public static boolean divides(int a, int b) throws IllegalArgumentException {
        ...
        //Above else statement will return true or false. No need to use an else-if and else statement.
        // Cleaner code
    }
    ...
    public static String message(int n) {
        //First check if the number is divisible by 15.
        // If not then goes through the out if - else statements
        if(n % 15 == 0){
            return n + ": " + "FizzBuzz";
        } else if (n % 3 == 0) {
            return n + ": " + "Fizz";
    ...
    public static String[] messages(int start, int end) throws IllegalArgumentException {
        ...
        //Question # 2
        // ?? Why is it underlined?  Reassigned local variable?
        String messagesInArray = "";
        for(int i = start; i < end; i ++ ){
            //ONLY concatenating FIZZ , BUZZ, FIZZBUZZ

            //Question # 2
            //?? First I was using space " " to concatenate and used space for a delimiter in the split function.
            // messagesInArray.split(",") --> [3: Fizz 5: Buzz 6: Fizz ]
            //First -> No Commas. Why?
            //Second -> I got one more space than needed.  Why?

            //When I use a "," to concatenate and used "," for a delimiter in the split function it would work fine. Why?
            // messagesInArray = 3: Fizz,5: Buzz,6: Fizz,
            // messagesInArray.split(",") --> [3: Fizz, 5: Buzz, 6: Fizz]

            if(message((i)) != null){
                messagesInArray += message(i) + ",";
            }
        }

        if(messagesInArray.isEmpty()){
            return new String[] {};
        }else {
            //Using split to turn the string into an array. Splitting by spaces. /s == space
            return messagesInArray.split(",");
        }
    }
    ...
    public static void main(String[] args) {
        for(int i = 1 ; i <= 115 ;i++ ){
            System.out.println(Arrays.toString(messages(i,115)));
        }
